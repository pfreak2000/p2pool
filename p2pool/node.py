...
class Node(object):
    ...
    def start(self):
        ...
        def handle_header(new_header, valid=False):
            new_hash = self.net.PARENT.POW_FUNC(starwels_data.block_header_type.pack(new_header))
            # check that header matches current target
            if new_hash > self.starwelsd_work.value['bits'].target:
                return
            if not valid:
                try:
                    _ = (yield self.starwelsd.rpc_getblockheader(new_hash))
                except:
                    return
        ...
        def poll_header():
            if self.factory.conn.value is None:
                return
            handle_header((yield self.factory.conn.value.get_block_header(self.starwelsd_work.value['previous_block'])), True)

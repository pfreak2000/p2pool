...
class WorkerBridge(worker_interface.WorkerBridge):
    ...
    def get_work(self, pubkey_hash, desired_share_target, desired_pseudoshare_target):
        ...
        if set(r[1:] if r.startswith('!') else r for r in self.node.starwelsd_work.value['rules']) - set(getattr(self.node.net, 'SOFTFORKS_REQUIRED', [])):
        ...
        packed_gentx = starwels_data.tx_id_type.pack(gentx) # stratum miners work with stripped transactions
        ...
        merkle_link = starwels_data.calculate_merkle_link([None] + other_transaction_hashes, 0) if share_info.get('segwit_data', None) is None else share_info['segwit_data']['txid_merkle_link']
        ...
        def got_response(header, user, coinbase_nonce):
            ...
            if starwels_data.is_segwit_tx(gentx): # reintroduce witness data to the gentx produced by stratum miners
                new_gentx['marker'] = 0

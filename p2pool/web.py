...
def get_web_root(wb, datadir_path, starwelsd_getinfo_var, stop_event=variable.Event(), static_dir=None):
    node = wb.node
    ...
    def get_global_stats():
        ...
        diff = starwels_data.target_to_difficulty(wb.current_work.value['bits'].target)
    ...
    def get_local_stats():
        ...
        for addr in wb.last_work_shares.value:
            miner_last_difficulties[addr] = starwels_data.target_to_difficulty(wb.last_work_shares.value[addr].target)
    ...
    web_root.putChild('payout_addrs', WebInterface(lambda: list(('%s' % starwels_data.pubkey_hash_to_address(add, node.net.PARENT)) for add in wb.pubkeys.keys)))
    ...
    def update_stat_log():
        ...
        for add in wb.pubkeys.keys:
            my_current_payout+=node.get_current_txouts().get(starwels_data.pubkey_hash_to_script2(add), 0)*1e-8
    ...
    def get_share_address(share_hash_str):
        if int(share_hash_str, 16) not in node.tracker.items:
            return None
        share = node.tracker.items[int(share_hash_str, 16)]
        return starwels_data.script2_to_address(share.new_script, node.net.PARENT)
    ...
    def add_point():
        ...
        for add in wb.pubkeys.keys:
             my_current_payouts += current_txouts.get(starwels_data.pubkey_hash_to_script2(add), 0)*1e-8
